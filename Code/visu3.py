    ...
    return [html.Div([
        dcc.RadioItems(
            ...
            options=[
                ...
                {'label': '  Seulement les concurrents', 'value': 'adverse-teams'}
            ],
            value='all-teams',
...
def update_output(selected_option_1: str, selected_option_2: str) -> list:
    if selected_option_1 == 'all-teams' :
        return [dcc.Graph(
                id='visu_3',
                figure=get_figure(
                    data_list=[DATA[DATA['Round'] == i],ADV[ADV['Team1'] == TEAM]], team=TEAM, criteria=selected_option_2, phase=i),
        ...
        return [dcc.Graph(
                id='visu_3',
                figure=get_figure_2(
                    data_list=[DATA[DATA['Squad'] == i],ADV[ADV['Team1'] == TEAM]], team=i, criteria=selected_option_2, phase=i),
                style={'width': '50%', 'margin': '0 auto'})
                for i in list_teams]


def get_figure(data_list: list[pd.DataFrame], team: str, criteria: str, phase: float) -> dict:
    data = data_list[0]
    scale = {'Score': [-0.5, 6.5],
             'Age': [15, 40],
             'Fatigue': [0, 101]}
    ...
    for x in list(data['Squad'].unique()):
        if x != team:
            fig.add_trace(go.Scatter(
                ...
                mode='markers',
                marker=dict(color='rgba(153,153,153,255)', size=10, line=dict(color='rgba(10, 10, 10, 1.0)', width=1)),
                hoverinfo='text',
...
def get_figure_2(data_list: list[pd.DataFrame], team: str, criteria: str, phase: float) -> dict:
    data = data_list[0]
    adv = data_list[1]
    ...
    if team != TEAM :
        ...
        fig.update_layout(
            title=str(to_replace[list(adv[adv['Team2'] == team]['Round'])[0]])+' : '+team,
            height=200,
            plot_bgcolor='rgba(0, 0, 0, 0)',
            yaxis=dict(showticklabels=False),
            margin=dict(l=10, r=10, t=50, b=5),
        ...
        fig.update_layout(
            title=str(to_replace[list(adv[adv['Team1'] == team]['Round'])[-1]])+' : '+team,
            height=200,
            plot_bgcolor='rgba(0, 0, 0, 0)',
            yaxis=dict(showticklabels=False),
            margin=dict(l=10, r=10, t=50, b=5),
